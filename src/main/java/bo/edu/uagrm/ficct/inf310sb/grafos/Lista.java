...
public class Lista {
    ...
    public void add(int data, double precio,double distancia){ //Inserta data a la Lista.
        Nodo Ant = null;
        Nodo p   = L;
        ...
        if (Ant == null){   //x es menor a todos los que están en la Lista (o L==null)
            nuevo = new Nodo(data, precio,distancia);
            nuevo.setLink(L);
            L = nuevo;
            n++;
        }
    ...
    public double getPrecio(int data){    //Devuelve el Peso que acompaña al data.  Si data no existe, devuelve 0.
        Nodo p = exist(data);
        if (p != null)
            return p.getPrecio();
    ...
    public String toString(){ 
        ...
        Nodo p  = L;
        while (p != null){
            S += coma + p.getData() + "/" + doubleToStr( p.getPrecio() )+ "/" +doubleToStr( p.getDistancia() );
            coma=", ";
            p = p.getLink();
        }
